...
class setting_config:
    ...
    datasets = 'CHN6_CUG'
    if datasets == 'CHN6_CUG':
        data_path = '/The path of CHN6_CUG dataset/'
    elif datasets == 'DeepGlobe':
        data_path = '/The path of DeepGlobe dataset/'
    else:
    ...
    # work_dir = 'results/' + datasets + '_' + datetime.now().strftime('%m_%d_%Hh_%Mm') + '/'
    ...
    train_transformer = transforms.Compose([
        myNormalize(datasets),
        myToTensor(),
        myRandomHorizontalFlip(p=0.5),
        myRandomVerticalFlip(p=0.5),
        myRandomRotation(p=0.5, degree=[0, 360]),
        myResize(resize_size, resize_size)
    ])
