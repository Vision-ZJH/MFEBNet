...
class myNormalize:
    ...
    def __call__(self, data):
        ...
        if self.data_name == 'CHN6_CUG':
            mean = [0.34923686, 0.3498158, 0.34459729]
            std = [0.15736567, 0.16971126, 0.19313258]
        elif self.data_name == 'DeepGlobe':
            mean = [0.28812228, 0.38283475, 0.40970726]
            std = [0.09659112, 0.105545, 0.12907971]
        ...
        imgs_normalized = augmented['image']
        return imgs_normalized, msk
...
class trainNormalize:
    ...
    def __call__(self, data):
        ...
        transform = A.Normalize(
            mean = mean,
            std = std,
            max_pixel_value=255,       # 图像的最大像素值，通常是255
        )
        augmented = transform(image=img)
        imgs_normalized = augmented['image']
        return imgs_normalized, msk
